...
function extractStyles(theme) {
    ...
    for (var elm in colors) {
        if (!colors[elm] && defaultGlobals[elm]) {
            colors[elm] = defaultGlobals[elm];
        }
    }
...
var themes = require('../../tmp/Colorsublime-Themes-master/themes.json').reduce((acc, theme) => {
    const id = theme.FileName.replace('.tmTheme', '')
    acc[id] = id;
    return acc;
}, {});
